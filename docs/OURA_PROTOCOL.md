...
- **[oura-rs]** - Th0rgal/open_oura `crates/oura-protocol/src/events.rs` (no-license Rust clean-room decoder; facts cited only, no code copied). Its own event tags marked `"_status": "unvalidated"` are treated the same as our Tier B - plausible, not ground-truth-confirmed.
...
- **`0x50` activity_info / `0x51`,`0x52` activity_summary**: activity category + intensity (MET-class). Layout **(UNVERIFIED - partial)**; [ringverse] notes real_steps/activity_info have unresolved constants. Gate on fixtures. [ringverse]
  - **`0x50` decode formula, live Gen 3 test, 2026-07-02 [oura-rs]:** byte0 = a `state` code (activity-category, meaning unconfirmed); remaining bytes = a per-sample MET series, `met = byte × 0.1` for `byte < 0x80`, else `met = 12.8 + (byte − 128) × 0.2`. **Plausible against 3 real Gen 3 captures** (21/21 decoded MET values landed in a sane 1.3-3.3 sedentary/light-activity range, nothing negative or absurd) but **NOT independently ground-truth-validated** (no known Oura-app MET/step comparison run). Stays Tier B: NOOP decodes it (`OuraDecoders.decodeActivityInfo`, `OuraEvent.activityInfo`) but gates it behind `allowTierB` and never folds it into `OuraStreamMapping`/scoring. `0x51`/`0x52` activity_summary and `0x7E`/`0x7F` real_steps remain fully undecoded (raw bytes only).
  - **Real Steps (feature `0x0B`) gating, [open_oura-feat]:** `"Gate: Gen3+; server flag activity/real_steps (default false); gated by FeatureDefinitions.ActivityRealSteps; disabled until server flag activates it."` This is the same server-flag-off pattern already confirmed for SpO2 (§7.1) and explains why `0x7E`/`0x7F` have never once appeared in three live sessions - not a NOOP decode gap. `0x50` activity/MET itself is NOT gated (always-on base stream per [open_oura-feat]), which matches it appearing in every session.
