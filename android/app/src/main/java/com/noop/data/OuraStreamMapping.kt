    ...
    fun streams(events: List<OuraEvent>, anchor: (Long) -> Int?): Streams {
        ...
        for (ev in events) {
            when (ev) {
                ...
                // Motion / state / time-sync / rtc / debug / TierB / ActivityInfo / CvaRawPpg /
                // RealStepsFields never map onto a scored stream. In particular the 0x50 activity/MET
                // decode (PR #960) NEVER mints a `steps` row: the formula is third-party and unvalidated
                // (Tier B, OURA_PROTOCOL.md s6.13), and MET is not a step count - fabricating one would
                // break the honest-data invariant and the per-source day-owner rules. Same discipline for
                // 0x81 CVA raw PPG (s6.14) and 0x7E/0x7F real_steps (s6.13) - decoded, logged, never
                // scored, not even real_steps' leading candidate field. 0x6A used to be dropped here too; it is
