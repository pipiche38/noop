    ...
    fun tierBAndActivityInfoNeverMapToAStream() {
        ...
        val s = OuraStreamMapping.streams(
            listOf(
                ...
                // 0x6A sleep_period_info is NOT in this list any more: `breath` now mints a resp row
                // (see sleepPeriodInfoMapsBreathToRespirationAndNothingElse, which also pins the half
                // that still holds - its `averageHrBpm` must never join the beat-derived HR series).
            ),
            anchor,
        )
        assertTrue(s.hr.isEmpty())
