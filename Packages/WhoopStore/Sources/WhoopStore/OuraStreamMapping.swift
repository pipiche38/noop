    ...
    public static func streams(from events: [OuraEvent], at ts: Int) -> Streams {
        ...
        for e in events {
            ...
            case .spo2(let v):
                ...
                // own `ts` and the anchor semantics are unchanged. This is the SAME treatment the 0x5D HRV
                // buckets get above and the hypnogram assembler gets below (`.sleepPhase`): a documented
                // cadence plus a record anchor, never a guessed time.
