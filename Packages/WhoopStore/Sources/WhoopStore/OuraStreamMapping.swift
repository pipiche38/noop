    ...
    public static func streams(from events: [OuraEvent], at ts: Int) -> Streams {
        ...
        for e in events {
            ...
            case .motion, .state, .timeSync, .rtcBeacon, .debugText, .tierB, .activityInfo, .cvaRawPpg,
                ...
                // leading candidate field (see OuraRealStepsFields), and the 0x50 activity/MET decode
                // NEVER mints one either: the formula is third-party and unvalidated, and MET is not a
                // step count — fabricating one would break the honest-data invariant.
                //
